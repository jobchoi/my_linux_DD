...
//#define GPIO_DEVICE             "gpiod"              /* 디바이스 장치 파일의 이름 */
#define GPIO_LED                10                         /* LED 사용을 위한 GPIO의 번호 */
#define GPIO_ON_SW              	24		       /* 스위치에 대한 GPIO의 번호 */
#define GPIO_OFF_SW                 25		       /* 스위치에 대한 GPIO의 번호 */
...
static int switch_on_irq;
static int switch_off_irq;
...
int GPIO_init(void)
{
    ...
    gpio_request(GPIO_ON_SW,  "ON  SWITCH");
    gpio_request(GPIO_OFF_SW, "OFF SWITCH");
    ...
    // GPIO 핀 IRQ 등록 
    switch_on_irq = gpio_to_irq(GPIO_ON_SW);
    switch_off_irq = gpio_to_irq(GPIO_OFF_SW);
    //request_irq(switch_irq, isr_func, IRQF_TRIGGER_RISING | IRQF_DISABLED, "switch", NULL);
    request_irq(switch_on_irq, isr_on_func, IRQF_TRIGGER_FALLING, "switch on", NULL);
    request_irq(switch_off_irq, isr_off_func, IRQF_TRIGGER_FALLING, "switch off", NULL);
...
void GPIO_exit(void)
{
    ...
    // 사용이 끝난 인터럽트 해제
    //=========================================================== 
    free_irq(switch_on_irq, NULL);
    free_irq(switch_off_irq, NULL);

    //===========================================================   
    // 더 이상 사용이 필요없는 경우 관련 자원을 해제한다.
    //=========================================================== 
    gpio_free(GPIO_LED);
    gpio_free(GPIO_ON_SW);
    gpio_free(GPIO_OFF_SW);
